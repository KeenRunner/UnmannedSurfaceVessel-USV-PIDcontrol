...
void change_point() {
    write_serial(0, 0);
    sleep(1);
    if (point_idx == point_nums - 1) {
        if (is_loop == 0)
            pos_confirm = 1;
        else {
            point_idx = 0;
            pos_obj[0] = point_list.at(point_idx*2);
            pos_obj[1] = point_list.at(point_idx*2 + 1);
            ang_confirm = 0;
            distance_sum = 0;
            ang_err_sum=0;
        }
...
{
    // 初始化ROS节点 创建节点句柄
    ros::init(argc, argv, "multi_point");
    ros::NodeHandle n;
    sleep(3);
    std::string serial_port;
    n.getParam("/multi_point/serial_port", serial_port);

    n.getParam("/multi_point/ang_kp_kd_ki_1", ang_kp_kd_ki_1);
    n.getParam("/multi_point/ang_kp_kd_ki_2", ang_kp_kd_ki_2);
    n.getParam("/multi_point/l_acc_factor", l_acc_factor);
    n.getParam("/multi_point/r_acc_factor", r_acc_factor);
    n.getParam("/multi_point/ang_dt", ang_dt);
    n.getParam("/multi_point/ang_pwm_max", ang_pwm_max);
    n.getParam("/multi_point/ang_thresh", ang_thresh);
    n.getParam("/multi_point/ang_vel_thresh", ang_vel_thresh);

    n.getParam("/multi_point/obj_p_list", point_list);
    n.getParam("/multi_point/amp_factor", amp_factor);
    n.getParam("/multi_point/pos_kp_kd_ki_1", pos_kp_kd_ki_1);
    n.getParam("/multi_point/pos_kp_kd_ki_2", pos_kp_kd_ki_2);
    n.getParam("/multi_point/distance_thresh", distance_thresh);
    n.getParam("/multi_point/pos_dt", pos_dt);
    n.getParam("/multi_point/linear_acc_max", linear_acc_max);

    n.getParam("/multi_point/pos_param_switch", pos_param_switch);
    n.getParam("/multi_point/ang_param_switch", ang_param_switch);
    n.getParam("/multi_point/is_loop", is_loop);

    point_nums = point_list.size() / 2;
